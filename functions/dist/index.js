...
.gallery img{width:100%;border-radius:12px;}`;
    ...
    const layout = (title, body) => `<!doctype html><html lang="${plan.locale}">
<head>
<meta charset="utf-8"/>
...
<footer>
  ...
  <p>${plan.contact?.city ?? ""}</p>
                ...
                case "cta":
                    return `<section><a class="btn" href="/pages/contact.html">${plan.locale === "fr" ? "Parler de votre projet" : "Start a project"}</a></section>`;
                default:
                    return "";
            }
        })
        ...
        const galleryHtml = plan.media?.gallery && plan.media.gallery.length
            ? `<section>
...
app.post("/generate", async (req, res) => {
    try {
        ...
        res.setHeader("Content-Disposition", `attachment; filename="${plan.site.name.replace(/\s+/g, "_")}.zip"`);
        res.send(buf);
    }
    catch (e) {
